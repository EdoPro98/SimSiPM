...
BENCHMARK_F(BenchmarkRandom, SingleRandomDouble)(benchmark::State& st) {
  for (auto _ : st) {
    double x;
    benchmark::DoNotOptimize(x = m_random.Rand());
  }
}
...
BENCHMARK_F(BenchmarkRandom, SingleRandomFloat)(benchmark::State& st) {
  for (auto _ : st) {
    float x;
    benchmark::DoNotOptimize(x = m_random.Rand<float>());
  }
}

BENCHMARK_F(BenchmarkRandom, PairRandomFloat)(benchmark::State& st) {
  for (auto _ : st) {
    sipm::pair<float, float> x;
    benchmark::DoNotOptimize(x = m_random.RandF2());
  }
}
...
BENCHMARK_F(BenchmarkRandom, SingleGuassianDouble)(benchmark::State& st) {
  for (auto _ : st) {
    double x;
    benchmark::DoNotOptimize(x = m_random.randGaussian(0, 1));
  }
}
...
BENCHMARK_F(BenchmarkRandom, SingleGuassianFloat)(benchmark::State& st) {
  for (auto _ : st) {
    float x;
    benchmark::DoNotOptimize(x = m_random.randGaussianF(0, 1));
  }
}
...
BENCHMARK_F(BenchmarkRandom, SingleInteger)(benchmark::State& st) {
  for (auto _ : st) {
    uint32_t x;
    benchmark::DoNotOptimize(x = m_random.randInteger(10));
  }
}
...
BENCHMARK_F(BenchmarkRandom, SingleExponential)(benchmark::State& st) {
  for (auto _ : st) {
    double x;
    benchmark::DoNotOptimize(x = m_random.randExponential(10));
  }
}
...
BENCHMARK_DEFINE_F(BenchmarkRandom, MultipleRandomDouble)(benchmark::State& st) {
  for (auto _ : st) {
    std::vector<double> x;
    benchmark::DoNotOptimize(x = m_random.Rand(st.range(0)));
  }
...
BENCHMARK_DEFINE_F(BenchmarkRandom, MultipleRandomFloat)(benchmark::State& st) {
  for (auto _ : st) {
    std::vector<float> x;
    benchmark::DoNotOptimize(x = m_random.RandF(st.range(0)));
  }
  st.SetComplexityN(st.range(0));
}
...
BENCHMARK_DEFINE_F(BenchmarkRandom, MultipleInteger)(benchmark::State& st) {
  for (auto _ : st) {
    ...
    benchmark::DoNotOptimize(x = m_random.randInteger(10, st.range(0)));
  }
}
...
BENCHMARK_DEFINE_F(BenchmarkRandom, MultipleGaussianDouble)(benchmark::State& st) {
  for (auto _ : st) {
    std::vector<double> x;
    benchmark::DoNotOptimize(x = m_random.randGaussian(0, 1, st.range(0)));
  }
}
...
BENCHMARK_DEFINE_F(BenchmarkRandom, MultipleGaussianFloat)(benchmark::State& st) {
  for (auto _ : st) {
    std::vector<float> x;
    benchmark::DoNotOptimize(x = m_random.randGaussianF(0, 1, st.range(0)));
  }
}
